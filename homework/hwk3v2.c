...
void findNeighbors(Box **qtree, Box **neighboringLeaves, Point p, double radius,
                   Box *currentBox, int *neighboringLeafIndex, int n, int k);
...
int circleInBox(Point p, double r, Box b);
...
int main(int argc, char *argv[]) {
  ...
  printf("Time to generate ranodoms: %f\n", time_spent);
...
void seek(double* a, int n, int k, int* iz) {
  ...
  *qtreeSize = n;
  ...
  Box **neighboringLeaves = malloc(leafCount * sizeof (Box *));
  ...
  for(i = 0; i < n; i++) {
    ...
    *neighboringLeafIndex = 0;
    ...
    findNeighbors(qtree, neighboringLeaves, points[i], radius, currentBox, neighboringLeafIndex, n, k);
...
void buildQtree(Box ***qtree, int *qtreeSize, Point *points, int n, int k,
  ...
  while(currentBoxIndex >= 0) {
    ...
    if(maxBoxIndex > (*qtreeSize) - 10) {
      (*qtreeSize) *= 2;
      Box **temp = malloc((*qtreeSize) * sizeof(Box*));
    ...
    if(currentBox->permutationEnd - currentBox->permutationStart > k) {
      ...
      child1->lowerLeftCorner.y = currentBox->lowerLeftCorner.y;
      child1->upperRightCorner.x = currentBox->lowerLeftCorner.x +((currentBox->upperRightCorner.x - currentBox->lowerLeftCorner.x)/2);
      child1->upperRightCorner.y = currentBox->upperRightCorner.y +((currentBox->upperRightCorner.y - currentBox->lowerLeftCorner.y)/2);
      ...
      child3->lowerLeftCorner.y = currentBox->lowerLeftCorner.y + ((currentBox->upperRightCorner.y - currentBox->lowerLeftCorner.y)/2);
      child3->upperRightCorner.x = currentBox->lowerLeftCorner.x + ((currentBox->upperRightCorner.x - currentBox->lowerLeftCorner.x)/2);
      child3->upperRightCorner.y = currentBox->lowerLeftCorner.y;
...
void sort(int *permutations, Point *points, int *indices, int permStart,
  ...
  for(i = permStart; i < permEnd; i++) {
    if(pointInBox(points[permutations[i]], child1)) {
      ...
      printf("Error placing points in boxes!!!!\n");
      exit(1);
    }
  }
...
void findNeighbors(Box **qtree, Box **neighboringLeaves, Point p, double radius,
                   Box *currentBox, int *neighboringLeafIndex, int n, int k) {
  while(currentBox->index != 0) {
    if (circleInBox(p, radius, *(currentBox))) {
      break;
    }
    currentBox = qtree[currentBox->parent];
  }

  Box **searchQueue = malloc(n * sizeof(Box *));
  int searchIndex = 1;

  searchQueue[0] = currentBox;
  ...
  for(i = 0; i < searchIndex; i++) {
    ...
    } else if(node->permutationStart != node->permutationEnd) { //leaf
...
int circleInBox(Point p, double r, Box b) {
  if (p.x - r >= b.lowerLeftCorner.x &&
    p.x + r <= b.upperRightCorner.x &&
    p.y - r >= b.lowerLeftCorner.y &&
    p.y + r <= b.upperRightCorner.y) {
    return 1;
  }
  return 0;
}
